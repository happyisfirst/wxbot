...
class group:
    ...
    def makeGroupDir(self):
        if(not os.path.exists(self._path)):
            os.mkdir(self._path)
            file = open(self._path + gl.PATH_SEPARATOR + 'total.txt', 'w')
            file.close()
    ...
    def classGroupMessagesToPerson(self):
        ...
        # 第一个用户
        currentUser = user(self.getUsername(self._messages[0]), self._groupname, self._keywords)
        ...
        for message in self._messages:
            ...
            if(currentUser.getUsername() != message_username 
                ...
                currentUser = user(message_username, self._groupname, self._keywords)
                self._members.append(currentUser)
    ...
    def writeTotalkeyInfo(self):
        # 总信息文件路径
        totalFilaPath = self._path + '/keywords.txt'
        if(not os.path.exists(totalFilaPath)):
            file = open(totalFilaPath, 'w')
            file.close()
        ...
        line = file.readline()
        while line:
            if(gl.TOTAL_KEYINFO_TITLE in line):
                line = file.readline()
                continue
            for index in range(len(line)):
                if(line[index] == ':'):
                    ...
                    userIndex = self.checkUserExist(username)
                    if(userIndex > -1):
                        totalInfo[username] = speakTimes + self._members[userIndex].getkeysnumInfo(self._keywords)
                        addedUser.append(userIndex)
                    else:
                        totalInfo[line[:index]] = speakTimes
                    break
        ...
        file.close()
        file = open(totalFilaPath, 'w')
        file.write(gl.TOTAL_KEYINFO_TITLE + '\n')
        ...
        for userIndex in range(len(self._members)):
            ...
            file.write(self._members[userIndex].getUsername()
                 + ':'
                 + str(self._members[userIndex].getkeysnumInfo(self._keywords)) + '\n')
        file.close()