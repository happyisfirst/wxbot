#这段代码原型主要是将定时发送的任务存储到数据库
#使得程序在加载时不必重复添加定时提醒的任务
#目前还没有做任何处理。
#coding:utf-8
...
scheduler.add_job(my_job, args=['job_interval',],id='job_interval',trigger='interval', seconds=5,replace_existing=True)#replace existing 不重复添加两个相同任务
